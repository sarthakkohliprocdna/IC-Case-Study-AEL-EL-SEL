...
with st.sidebar:
    ...
    uploaded_narrative = st.file_uploader("Upload narrative / deck export", type=["txt", "csv", "xlsx", "xls", "docx", "pdf"], key="narrative")
    uploaded_workbook = st.file_uploader("Upload candidate workbook", type=["xlsx", "xls"], key="workbook")
...
if evaluate:
    ...
    if narrative_score is not None:
        base_for_combined = narrative_score
    else:
        base_for_combined = workbook_score

    combined, weighting_note = combined_score(narrative_score or workbook_score, workbook_score, role_level) if narrative_score is not None else (workbook_score, "Workbook only")
    ...
    if narrative_result:
        ...
        for s in narrative_result["dimension_scores"]:
            rows.append({"Dimension": s.name, "Weight": s.weight, "Raw score": s.raw_score, "Weighted score": s.weighted_score, "Band": s.band, "Commentary": s.commentary})
    ...
    if workbook_result:
        ...
        for k, v in workbook_result["component_scores"].items():
            wc_rows.append({"Component": k, "Weight": workbook_result["weights"][k], "Score": v, "Band": score_to_band(v)})
    ...
    st.write("The workbook must contain `Candidate_Goals` and `Candidate_Design` sheets.")
    st.dataframe(pd.DataFrame([
        {"Sheet": "Candidate_Goals", "Required columns": "Territory_ID, Candidate_Q6_Goal"},
        {"Sheet": "Candidate_Design", "Required columns": "Parameter, Value"},
    ]), use_container_width=True, hide_index=True)
    st.markdown("## Rubric dimensions")
    st.dataframe(pd.DataFrame([{"Dimension": d["name"], "Weight": d["weight"], "Description": d["description"]} for d in rubric.get("dimensions", [])]), use_container_width=True, hide_index=True)
